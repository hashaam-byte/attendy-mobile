...
type Student = { id:string; full_name:string; class_name:string|null; parent_phone:string|null; employee_id:string|null; is_active:boolean; notes:string|null; created_at:string };
...
export default function StudentProfileScreen({ route }: any) {
  ...
  const load = useCallback(async (refresh=false) => {
    ...
    const [{data:s},{data:l}] = await Promise.all([
      supabase.from('members').select('id,full_name,class_name,parent_phone,employee_id,is_active,notes,created_at').eq('id',studentId).eq('organisation_id',authState.orgId).single(),
      supabase.from('attendance_logs').select('id,scanned_at,status,scan_type,late_reason').eq('member_id',studentId).eq('scan_type','entry').order('scanned_at',{ascending:false}).limit(60),
    ]);
    if (s) setStudent(s);
    setLogs(l??[]); setLoading(false); setRefreshing(false);
  ...
  function handleSuspend() {
    ...
    Alert.alert(student.is_active?'Suspend Student?':'Reactivate Student?',
      student.is_active?`${student.full_name}'s QR card will be rejected at the gate.`:`${student.full_name} will be able to scan in again.`,
      [{text:'Cancel',style:'cancel'},{text:student.is_active?'Suspend':'Reactivate',style:student.is_active?'destructive':'default',onPress:async()=>{
        await supabase.from('members').update({is_active:!student.is_active}).eq('id',student.id).eq('organisation_id',authState.orgId);
  ...
  if (!student) return <View style={{flex:1,backgroundColor:theme.bg,alignItems:'center',justifyContent:'center'}}><Text style={{color:theme.text}}>Student not found</Text></View>;
  ...
  const pct = logs.length>0 ? Math.round((attended/logs.length)*100) : 0;
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>load(true)} tintColor={c}/>}>
        <View style={[styles.profileHeader,{backgroundColor:`${c}08`,borderBottomColor:theme.border}]}>
          ...
          {student.class_name&&(
            <View style={[styles.classBadge,{backgroundColor:`${c}15`,borderColor:`${c}30`}]}>
              <Ionicons name="school-outline" size={11} color={c}/><Text style={[styles.classText,{color:c}]}>{student.class_name}</Text>
            </View>
          )}
          <View style={[styles.statusBadge,{backgroundColor:student.is_active?theme.successBg:theme.dangerBg}]}>
            <Text style={{fontSize:FONT.sm,fontWeight:'700',color:student.is_active?theme.success:theme.danger}}>{student.is_active?'Active':'Suspended'}</Text>
          </View>
        </View>

        {student.parent_phone&&(
          <View style={styles.contactRow}>
            <TouchableOpacity style={[styles.contactBtn,{backgroundColor:theme.successBg,borderColor:`${theme.success}30`}]} onPress={()=>Linking.openURL(`tel:${student.parent_phone}`)}>
              <Ionicons name="call-outline" size={16} color={theme.success}/><Text style={[styles.contactBtnText,{color:theme.success}]}>Call Parent</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.contactBtn,{backgroundColor:theme.successBg,borderColor:`${theme.success}30`}]} onPress={()=>{
              ...
              <Ionicons name="logo-whatsapp" size={16} color={theme.success}/><Text style={[styles.contactBtnText,{color:theme.success}]}>WhatsApp</Text>
            </TouchableOpacity>
          </View>
        )}

        <View style={styles.statsGrid}>
          {[{l:'Present',v:presentCount,c:theme.success},{l:'Late',v:lateCount,c:theme.warn},{l:'Excused',v:excusedCount,c:theme.info},{l:'Rate',v:`${pct}%`,c:pct>=75?theme.success:theme.danger}].map(({l,v,c:col})=>(
        ...
        <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border}]}>
          <View style={styles.barHeader}>
            ...
            <Text style={[styles.barPct,{color:pct>=75?theme.success:theme.danger}]}>{pct}%</Text>
          </View>
          ...
          {pct<75&&(
            <View style={styles.warningRow}><Ionicons name="warning-outline" size={12} color={theme.danger}/><Text style={[styles.warningText,{color:theme.danger}]}>Below 75% — may affect exam eligibility</Text></View>
          )}
        </View>

        <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border,marginTop:0}]}>
          {[{label:'Student ID',value:student.employee_id},{label:'Parent Phone',value:student.parent_phone},{label:'Registered',value:formatDate(student.created_at)},{label:'Notes',value:student.notes}].filter(r=>r.value).map(({label,value},i,arr)=>(
        ...
        {authState?.role==='admin'&&(
          <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border,marginTop:0}]}>
            <TouchableOpacity style={styles.actionRow} onPress={handleSuspend} activeOpacity={0.6}>
              <Ionicons name={student.is_active ? ('shield-off-outline' as any) : ('shield-checkmark-outline' as any)} size={18} color={student.is_active?theme.warn:theme.success}/>
              <Text style={[styles.actionText,{color:student.is_active?theme.warn:theme.success}]}>{student.is_active?'Suspend Student':'Reactivate Student'}</Text>
        ...
        <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border,marginTop:0,marginBottom:32}]}>
          <Text style={[styles.historyTitle,{color:theme.text}]}>Attendance History ({logs.length})</Text>
          {logs.length===0 ? (
            <View style={{padding:24,alignItems:'center'}}><Text style={{fontSize:FONT.sm,color:theme.textMuted}}>No records yet</Text></View>
            ...
            return (
              <View key={log.id} style={[styles.logRow,i<Math.min(logs.length,20)-1&&{borderBottomWidth:1,borderBottomColor:theme.border}]}>
                <View style={[styles.logDot,{backgroundColor:`${cfg.color}18`,borderColor:`${cfg.color}40`}]}><Ionicons name="scan-outline" size={12} color={cfg.color}/></View>
                <View style={{flex:1}}>
                  <Text style={[styles.logDate,{color:theme.text}]}>{formatDate(log.scanned_at)}</Text>
                  {log.late_reason&&<Text style={[styles.logReason,{color:theme.textMuted}]}>{log.late_reason}</Text>}
                </View>
                <Text style={[styles.logTime,{color:theme.textMuted}]}>{formatTime(log.scanned_at)}</Text>
                <View style={[styles.logBadge,{backgroundColor:`${cfg.color}15`}]}><Text style={[styles.logBadgeText,{color:cfg.color}]}>{cfg.label}</Text></View>
              </View>
            );
          })}
          {logs.length>20&&<Text style={[styles.moreText,{color:theme.textMuted}]}>+{logs.length-20} more records on web dashboard</Text>}
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  barHeader:{flexDirection:'row',justifyContent:'space-between',padding:SPACING.md,paddingBottom:8},
  barTitle:{fontSize:FONT.sm},
