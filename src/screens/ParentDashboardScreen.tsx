...
type Student = { id:string; full_name:string; class_name:string|null; organisation_id:string; parent_phone:string|null; organisations:{name:string;primary_color:string}|null };
...
export default function ParentDashboardScreen({ navigation, route }: any) {
  ...
  const fetchLogs = useCallback(async (refresh=false) => {
    ...
    const { data } = await supabase.from('attendance_logs').select('id,scanned_at,status,scan_type,late_reason')
      .eq('member_id',selected.id).eq('scan_type','entry').order('scanned_at',{ascending:false}).limit(60);
    setLogs(data??[]); setLoading(false); setRefreshing(false);
  }, [selected?.id]);
    ...
    <View style={{flex:1,backgroundColor:theme.bg}}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>fetchLogs(true)} tintColor={primary}/>}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:SPACING.lg,gap:8,paddingVertical:SPACING.md}}>
            {students.map((s:Student,i:number)=>(
              <TouchableOpacity key={s.id} onPress={()=>setSelectedIdx(i)}
                ...
                <Text style={[styles.tabText,{color:selectedIdx===i?'white':theme.textSub}]}>{s.full_name.split(' ')[0]}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        )}
              ...
              {todayLog ? (()=>{
                ...
                return (
                  <View style={[styles.todayBadge,{backgroundColor:cfg.bg,borderColor:`${cfg.color}50`}]}>
                    ...
                    <Text style={[styles.todayBadgeText,{color:cfg.color}]}>{cfg.label} today · {formatTime(todayLog.scanned_at)}</Text>
                  </View>
                );
              })() : (
        ...
        <View style={styles.statsGrid}>
          {[{l:'On time',v:presentCount,c:theme.success},{l:'Late',v:lateCount,c:theme.warn},{l:'Excused',v:excusedCount,c:theme.info},{l:'Rate',v:`${pct}%`,c:pct>=75?theme.success:theme.danger}].map(({l,v,c})=>(
        ...
        <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border,margin:SPACING.lg,marginTop:0}]}>
          <View style={styles.barHeader}>
            ...
            <Text style={[styles.barPct,{color:pct>=75?theme.success:theme.danger}]}>{pct}% {pct>=75?'✓ Good':'⚠ Below 75%'}</Text>
          </View>
          <View style={[styles.barTrack,{backgroundColor:theme.bgCardAlt}]}>
            <View style={[styles.barFill,{width:`${Math.min(pct,100)}%` as any,backgroundColor:pct>=75?primary:theme.danger}]}/>
          </View>
          <Text style={[styles.barSub,{color:theme.textMuted}]}>Based on {total} recorded days</Text>
        </View>
        ...
        <View style={[styles.card,{backgroundColor:theme.bgCard,borderColor:theme.border,margin:SPACING.lg,marginTop:0}]}>
          ...
          {loading ? <View style={{padding:32,alignItems:'center'}}><ActivityIndicator color={primary}/></View>
          : logs.length===0 ? (
            ...
            return (
              <View key={log.id} style={[styles.logRow,i<29&&{borderBottomWidth:1,borderBottomColor:theme.border}]}>
                <View style={[styles.logDot,{backgroundColor:cfg.bg,borderColor:`${cfg.color}50`}]}><Ionicons name={cfg.icon as any} size={13} color={cfg.color}/></View>
                <View style={{flex:1}}>
                ...
                <View style={[styles.logBadge,{backgroundColor:cfg.bg}]}><Text style={[styles.logBadgeText,{color:cfg.color}]}>{cfg.label}</Text></View>
              </View>
            );
          })}
        </View>
        ...
        <View style={{padding:24,paddingTop:8,alignItems:'center'}}>
          <Text style={[styles.footerText,{color:theme.textMuted}]}>For queries, contact {org?.name??'your school'} directly.</Text>
        </View>
      </ScrollView>
    </View>
  );
